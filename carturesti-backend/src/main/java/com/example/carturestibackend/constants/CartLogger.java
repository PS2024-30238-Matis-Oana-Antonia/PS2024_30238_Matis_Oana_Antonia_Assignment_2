
package com.example.carturestibackend.constants;

public class CartLogger {
    public static final String CART_NOT_FOUND_BY_ID = "Cart with id {} was not found in the database";
    public static final String CART_RETRIEVED_BY_ID = "Cart with id {} retrieved from the database";
    public static final String CART_INSERTED = "Cart with id {} was inserted into the database";
    public static final String CART_DELETED = "Cart with id {} was deleted from the database";
    public static final String CART_UPDATED = "Cart with id {} was updated in the database";
    public static final String ALL_CARTS_RETRIEVED = "All carts retrieved successfully";
}
